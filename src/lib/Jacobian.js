...
import MatrixRmn from './MatrixRmn';
...
export default class Jacobian {
  ...
  Jend: MatrixRmn = new MatrixRmn(); // Jacobian matrix based on end effector positions
  Jtarget: MatrixRmn = new MatrixRmn(); // Jacobian matrix based on target positions
  Jnorms: MatrixRmn = new MatrixRmn(); // Norms of 3-vectors in active Jacobian (SDLS only)

  U: MatrixRmn = new MatrixRmn(); // J = this.U * Diag(this.w) * this.V^T  (Singular Value Decomposition)
  w: VectorRn = new VectorRn();
  V: MatrixRmn = new MatrixRmn();

  CurrentUpdateMode: UpdateMode;

  dS: VectorRn = new VectorRn(); // delta s
  dT1: VectorRn = new VectorRn(); // delta t    --  these are delta S values clamped to smaller magnitude
  dSclamp: VectorRn = new VectorRn(); // Value to clamp magnitude of dT at.
  dTheta: VectorRn = new VectorRn(); // delta theta
  dPreTheta: VectorRn = new VectorRn(); // delta theta for single eigenvalue  (SDLS only)

  errorArray: VectorRn = new VectorRn(); // Distance of end effectors from target after updating
  ...
  Jactive: MatrixRmn = new MatrixRmn();
  ...
  CalcDeltaThetasDLS(): void {
    ...
    // Scale back to not exceed maximum angle changes
    let maxChange = this.dTheta.MaxAbs();
    if (maxChange > MaxAngleDLS) {
      this.dTheta.multiply(MaxAngleDLS / maxChange);
    }
  }
  ...
  CalcDeltaThetasSDLS(): void {
    ...
    let i;
    let jx /*J*/ = 0;
    let jnx /*this.Jnorms*/ = 0;
    for (i = nCols * numEndEffectors; i > 0; i--) {
      let accumSq = Square(J[jx++]);
      accumSq += Square(J[jx++]);
      accumSq += Square(J[jx++]);
      this.Jnorms[jnx++] = Math.sqrt(accumSq);
    }

    // Clamp the this.dS values
    this.CalcdTClampedFromdS();
    ...
    for (i = 0; i < nRows; i++) {
      ...
      let N = 0.0; // N is the quasi-1-norm of the i-th column of this.U
      let alpha = 0.0; // alpha is the dot product of dT and the i-th column of this.U

      let dTx /*this.dT1*/ = 0;
      let ux /*this.U*/ = this.U.GetColumnPtrOffset(i);
      let j;
      for (j = numEndEffectors; j > 0; j--) {
        let tmp;
        alpha += this.U[ux] * this.dT1[dTx++];
        tmp = Square(this.U[ux++]);
        alpha += this.U[ux] * this.dT1[dTx++];
        tmp += Square(this.U[ux++]);
        alpha += this.U[ux] * this.dT1[dTx++];
        tmp += Square(this.U[ux++]);
      ...
      let M = 0.0;
      let vx /*this.V*/ = this.V.GetColumnPtrOffset(i);
      jnx /*this.Jnorms*/ = 0;
      for (j = nCols; j > 0; j--) {
        let accum = 0.0;
        for (let k = numEndEffectors; k > 0; k--) {
          accum += this.Jnorms[jnx++];
        }
        M += Math.abs(this.V[vx++]) * accum;
      }
      M *= Math.abs(wiInv);

      let gamma = MaxAngleSDLS;
      if (N < M) {
        gamma *= N / M; // Scale back maximum permissable joint angle
      }

      // Calculate the this.dTheta from pure pseudoinverse considerations
      let scale = alpha * wiInv; // This times i-th column of this.V is the psuedoinverse response
      this.dPreTheta.LoadScaledMatrixRmn(
        this.V,
        this.V.GetColumnPtrOffset(i),
        scale
      );
      // Now rescale the this.dTheta values.
      let max = this.dPreTheta.MaxAbs();
      let rescale = gamma / (gamma + max);
    ...
    if (maxChange > MaxAngleSDLS) {
      this.dTheta.multiply(MaxAngleSDLS / (MaxAngleSDLS + maxChange));
      //this.dTheta *= MaxAngleSDLS/maxChange;
    }
  }

  CalcdTClampedFromdS(): void {
    let len = this.dS.GetLength();
    let j = 0;
    for (let i = 0; i < len; i += 3, j++) {
      let normSq =
        Square(this.dS[i]) + Square(this.dS[i + 1]) + Square(this.dS[i + 2]);
      if (normSq > Square(this.dSclamp[j])) {
        let factor = this.dSclamp[j] / Math.sqrt(normSq);
