...
import {Quaternion} from './Spherical';
...
export class VectorR3 {
  ...
  toString() {
    return (
      '<' + String(this.x) + ',' + String(this.y) + ',' + String(this.z) + '>'
    );
  }
  ...
  times(u: number | VectorR3, m: number | VectorR3) {
    if (u instanceof VectorR3) {
      if (m instanceof VectorR3) {
        const v = m;
        return new VectorR3(
          u.y * v.z - u.z * v.y,
          u.z * v.x - u.x * v.z,
        ...
        return new VectorR3(u.x * m, u.y * m, u.z * m);
      }
    } else {
      if (m instanceof VectorR3) {
        return new VectorR3(m.x * u, m.y * u, m.z * u);
      } else {
        return u * m; //wat
      }
    }
  ...
  //   let sinhalf = Math.sqrt(Square(q.x) + Square(q.y) + Square(q.z));
  //   if (sinhalf > 0.0) {
  //     let theta = Math.atan2(sinhalf, q.w);
  //     theta += theta;
  //     this.Set(q.x, q.y, q.z);
  //     this.timesEqual(theta / sinhalf);
  //   } else {
  //     this.SetZero();
  //   }
  //   return this;
